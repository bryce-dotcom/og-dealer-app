  ...
  persist(
    (set, get) => ({
      ...
      fetchAllData: async () => {
        ...
        const [dealerRes, invRes, empRes, bhphRes, dealsRes, custRes] = await Promise.all([
          supabase.from('dealer_settings').select('*').eq('id', dealerId).single(),
        ...
        set({
          dealer: dealerRes.data || get().dealer,
    ...
    }),
    {
      ...
      partialize: (state) => ({ dealerId: state.dealerId, dealer: state.dealer })
    }
  )
);