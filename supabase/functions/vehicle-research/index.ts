...
serve(async (req) => {
  ...
  try {
    ...
    // =======================================================================
    // STEP 5: Use MarketCheck values only — no guessing
    // =======================================================================
    let finalValues = {
    ...
    let dataSource = finalValues.retail ? 'MarketCheck' : 'MarketCheck unavailable';

    if (!finalValues.retail) {
      log('MAIN', 'MarketCheck returned no values — not fabricating estimates');
    }
