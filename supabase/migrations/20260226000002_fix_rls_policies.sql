...
-- Overload for integer dealer_id
CREATE OR REPLACE FUNCTION user_has_dealer_access(check_dealer_id integer)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM dealer_settings
...
-- Overload for bigint dealer_id
CREATE OR REPLACE FUNCTION user_has_dealer_access(check_dealer_id bigint)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM dealer_settings
    WHERE id = check_dealer_id::integer
    AND (
      owner_user_id = auth.uid()
      OR EXISTS (
        SELECT 1 FROM employees
        WHERE dealer_id = check_dealer_id::integer
        AND user_id = auth.uid()
      )
    )
  );
...
COMMENT ON FUNCTION user_has_dealer_access(uuid) IS 'Helper function to check if authenticated user can access a dealer (owner or employee)';
